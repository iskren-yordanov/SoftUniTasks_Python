...
def explore_area(row, col, matrix):
    ...
    if matrix[row][col] != '-': # wall or visited
        return 0
    ...
    #we dont need to unmark the visited ones, since they are in the shape we worked on
...
for row in range(rows):
    for col in range(cols):
        ...
        #areas.append((row, col, size)) # tuple
